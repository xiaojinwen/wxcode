...
public class LoginHttpServer extends NanoHTTPD {
    ...
    // ========== 首页 HTML 缓存（配置变更时自动刷新） ==========

    /** 缓存的 HTML 模板（含 {SERVER_YEAR}/{SERVER_TIME} 占位符） */
    private volatile String cachedHtmlTemplate;
    /** 缓存对应的配置版本号 */
    private volatile long htmlCacheGeneration = -1;
    ...
    /** 暴露 ServerContext，供 WxLoginHook 在配置变更时递增 configGeneration */
    public ServerContext getServerContext() {
        return ctx;
    }

    /**
     * 统一构建 JSON 错误响应，避免手动拼接导致注入和格式错误。
     */
    private static String buildJsonError(int errCode, String message) {
        try {
            ...
            return "{\"err\":-500,\"msg\":\"internal error\"}";
        }
    }
    ...
    public NanoHTTPD.Response serve(NanoHTTPD.IHTTPSession iHTTPSession) {
        if (!iHTTPSession.getMethod().equals(NanoHTTPD.Method.GET)) {
            return jsonResponse(Response.Status.NOT_FOUND, buildJsonError(-404, "接口不存在"));
        }
        ...
        if (uri.equals("/whoami")) {
            try {
                ...
                return jsonResponse(Response.Status.OK, info.toString());
            } catch (Exception e) {
                return jsonResponse(Response.Status.INTERNAL_ERROR, buildJsonError(-500, e.getMessage()));
            }
        }
        ...
        if (uri.equals("/register")) {
            try {
                ...
                if (portStr != null && userIdStr != null) {
                    ...
                    return jsonResponse(Response.Status.OK, r.toString());
                }
                return jsonResponse(Response.Status.BAD_REQUEST, buildJsonError(-1, "missing port/userId"));
            } catch (Exception e) {
                return jsonResponse(Response.Status.INTERNAL_ERROR, buildJsonError(-500, e.getMessage()));
            }
        }
        ...
        if (uri.equals("/config")) {
            ...
            try {
                ...
                return jsonResponse(Response.Status.OK, cfg.toString());
            } catch (Exception e) {
                return jsonResponse(Response.Status.INTERNAL_ERROR, buildJsonError(-500, e.getMessage()));
            }
        }
        ...
        if (uri.equals("/config_raw")) {
            try {
                ...
                return NanoHTTPD.newFixedLengthResponse(Response.Status.OK, "application/json; charset=utf-8", prettyJson);
            } catch (Exception e) {
                return NanoHTTPD.newFixedLengthResponse(Response.Status.OK, "application/json; charset=utf-8", ctx.jsonString);
            }
        }
        ...
        if (uri.equals("/save_config")) {
            try {
                ...
                if ("save".equals(action)) {
                    ...
                    return jsonResponse(Response.Status.BAD_REQUEST, buildJsonError(-1, "缺少config参数"));
                    ...
                    return jsonResponse(Response.Status.BAD_REQUEST, buildJsonError(-1, "缺少必要参数(version/j1/c)"));
                    ...
                    if (version != null && !version.isEmpty()) {
                        ...
                        return jsonResponse(Response.Status.BAD_REQUEST, buildJsonError(-1, "版本 " + version + " 不存在"));
                    }
                    return jsonResponse(Response.Status.BAD_REQUEST, buildJsonError(-1, "缺少version参数"));
                ...
                return jsonResponse(Response.Status.BAD_REQUEST, buildJsonError(-1, "未知操作"));
            } catch (Exception e) {
                return jsonResponse(Response.Status.INTERNAL_ERROR, buildJsonError(-500, e.getMessage()));
            }
        }
        ...
        // 首页HTML页面 — 使用缓存，仅在配置变更时重建
        return htmlResponse(getCachedHomePage());
    }

    // ========== 便捷响应方法 ==========

    private static NanoHTTPD.Response jsonResponse(Response.Status status, String json) {
        return NanoHTTPD.newFixedLengthResponse(status, "application/json", json);
    }

    private static NanoHTTPD.Response htmlResponse(String html) {
        return NanoHTTPD.newFixedLengthResponse(Response.Status.OK, NanoHTTPD.MIME_HTML, html);
    }

    /**
     * 获取缓存的首页 HTML。比对 ctx.configGeneration 判断是否需要重建。
     * 多个线程可能同时进入并重建，但这是无害的（最终一致），且配置变更极其低频。
     */
    private String getCachedHomePage() {
        long currentGen = ctx.configGeneration;
        String cached = cachedHtmlTemplate;
        if (cached != null && htmlCacheGeneration == currentGen) {
            // 缓存有效，仅替换动态占位符
            return cached.replace("{SERVER_YEAR}", getCurrentYear())
                    .replace("{SERVER_TIME}", getCurrentTimeString());
        }
        // 重建缓存（配置已变更或首次访问）
        try {
            JSONObject config = new JSONObject(ctx.jsonString);
            cachedHtmlTemplate = buildHomePageHtml(config);
            htmlCacheGeneration = currentGen;
            XposedBridge.log("xiaojw-wxcode" + " 首页HTML缓存已刷新 gen=" + currentGen);
            return cachedHtmlTemplate.replace("{SERVER_YEAR}", getCurrentYear())
                    .replace("{SERVER_TIME}", getCurrentTimeString());
        } catch (Exception e) {
            XposedBridge.log("xiaojw-wxcode" + " 首页HTML构建失败: " + e.getMessage());
            return "<html><body><h1>服务器内部错误</h1></body></html>";
        }
    }
    ...
    /**
     * 构建首页 HTML（仅在配置变更或首次访问时调用）。
     * 使用 {SERVER_TIME} 占位符代替动态时间，每次请求时替换。
     */
    private String buildHomePageHtml(JSONObject jSONObject) throws Exception {
        ...
        while (itKeys.hasNext()) {
            ...
            if ("common".equals(next)) continue;
        ...
        sb.append("© {SERVER_YEAR} wxcode 插件 | 服务器时间：{SERVER_TIME}");
        sb.append("</div>");
        sb.append("</div>");
        sb.append("</body>");
        sb.append("</html>");
